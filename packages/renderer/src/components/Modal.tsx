...
export const Modal: FC<ModalProps> = ({
	...
	const triggerElement = cloneElement(trigger, { id: key, onClick: () => onVisibilityChange?.(true) });
					...
					<div className={contentCss}>
						<div className="p-3 w-full flex items-center shrink-0 border-b border-gray-700">
							...
							{showCloseIcon && canClose && (
								<button onClick={closeModal} className="ml-auto size-6 flex items-center justify-center text-gray-400 hover:text-white rounded-xs transition-colors" type="button">
									<Icon path={mdiClose} className="size-5"/>
								</button>
							)}
						</div>
						<div className="p-3 overflow-y-auto [scrollbar-width:thin]">{children}</div>
						{footer}
					</div>
