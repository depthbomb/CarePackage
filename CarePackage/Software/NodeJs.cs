...
public class NodeJs : BaseSoftware
{
    public override string           Key            { get; set; } = "nodejs-current";
    public override string           Name           { get; set; } = "Node.js";
    public override SoftwareCategory Category       { get; set; } = SoftwareCategory.Runtime;
    public override string           DownloadName   { get; set; } = "node-x64.msi";
    ...
    public NodeJs(IServiceProvider services)
    {
    ...
    public override async Task<string> GetDownloadUrlAsync(CancellationToken ct)
    {
        ...
        var version = matches
                      .OrderByDescending(m => new Version(m.Groups[1].Value[1..]))
                      .FirstOrDefault()
                      ?.Groups[1].Value;
